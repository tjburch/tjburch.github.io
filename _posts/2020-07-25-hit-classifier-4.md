...
This model can be used to predict hit outcomes, using just hit kinematics, player speed, and factors of the park you expect to be playing in. Because of this, it can be used for prediction in several regimes. However, it's very important to not make out-of-sample predictions, using the model somewhere it isn't trained to. This model was specifically trained on MLB data, so the domain of applicability is _only_ for MLB-like scenarios. Due to differences in pitching skill, it won't translate one-to-one to minor league (MILB) data.
